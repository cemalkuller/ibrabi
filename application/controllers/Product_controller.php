...
class Product_controller extends Home_Core_Controller
{
    ...
    public function start_selling_post()
    {
        ...
        */
        ...
        $birtyear = $this->input->post('birtyear', true);

        if (tc_kimlik_no_sorgula::tcKimlikNo($tc_no)->ad($first_name)->soyad($last_name)->dogumYili($birtyear)->sorgula())
        {
            //validate inputs
            $this->form_validation->set_rules('tc_no', 'TC No', 'required|xss_clean');
            $this->form_validation->set_rules('first_name', trans('TC No'), 'required|xss_clean');
            $this->form_validation->set_rules('last_name', trans('last_name'), 'required|xss_clean');
            $this->form_validation->set_rules('birtyear', trans('birtyear'), 'required|xss_clean');
            ...
            if ($this->form_validation->run() === false)
            {
                ...
                /*$data = array(
                    'email' => remove_special_characters($this->input->post('email', true)),
                    'first_name' => $this->input->post('first_name', true),
                    'last_name' => $this->input->post('last_name', true),
                ...
                if ($this->auth_model->add_shop_opening_requests()) {
                    //send email
            ...
            $this->session->set_flashdata('error', 'Ad soyad bilgileriniz yanlış veya Tc Kimlik numaranız geçersiz.');
            redirect($this->agent->referrer());
        }
    }
