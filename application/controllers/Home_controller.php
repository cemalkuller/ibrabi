...
class Home_controller extends Home_Core_Controller
{
    ...
    public function index()
    {
        ...
        $data['main_banners'] = $this->banner_model->get_banner_items(1);
        $data['sidebar_banners'] = $this->banner_model->get_banner_items(2);
        $data['brands'] = $this->banner_model->get_banner_items(3);
        //latest searchs
        $data['latest_searchs'] = $this->product_model->latest_searchs();
    ...
    {
        get_method();
        $data['title'] = 'Satış Yap';
        $data['description'] = 'Satış Yap' . " - " . $this->app_name;
        $data['keywords'] = 'Satış Yap' . "," . $this->app_name;
        $data['index_settings'] = $this->settings_model->get_index_settings();
        $this->load->view('partials/_header', $data);
        $this->load->view('start_selling', $data);
        $this->load->view('partials/_footer');
    }
    ...
    private function page($page)
    {
        ...
        if ($page->visibility == 0) {
            ...
            $data['page'] = $page;
            if ($page->page_default_name == 'blog') {
                $this->blog($data);
    ...
    public function search()
    {
        ...
        if ($search_type == 'product') {
            $this->product_model->insert_search_keyword($search);
