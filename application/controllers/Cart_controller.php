...
class Cart_controller extends Home_Core_Controller
{
     ...
     * Add to Cart
     */
    public function add_to_cart_direct($product_id)
    {
        $product = $this->product_model->get_product_by_id($product_id);
        if (!empty($product)) {
            ...
            $this->cart_model->add_to_cart_direct($product);
            redirect(lang_base_url() . 'cart');
        }
        redirect($this->agent->referrer());
    }
     ...
     * 
     */
    public function artkargo()
    {
        $api = new artkargo();
		$api->setApiKey('6344991732DD86FCF28CFC7D5C855418')/* restapi -> class ı içindede tanımlanabilir. ARTKARGO'dan talep ediniz. */
			->setReturnType('json');/* gönderilmez ise default olarak 'json' dur. istenilirse 'array' gönderilerek php array olarak dönüş alınabilir. */
		/* yeni kargo gönderme api testi yukarıdaki tarnımlı ayarı kullanır.*/
		...
		$response = $api->setParams($prms)/* methodun parametreleri */
		...
		print_r($response);
		echo $response['data']['takip_no']; exit;
    }
