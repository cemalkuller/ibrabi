...
class Auth_model extends CI_Model
{
    ...
    public function add_shop_opening_requests()
    {
        if ($this->is_logged_in()) {
            ...
            $this->save_user_for_opening_request();
            return true;
        }
    }

    //save user info
    public function save_user_for_opening_request()
    {
        $this->load->library('bcrypt');
        ...
        $data['tc_no'] = $this->input->post('tc_no', true);
        $data['store_category'] = $this->input->post('store_category', true);
        //secure password
        $data['password'] = $this->bcrypt->hash_password($this->input->post('password', true));
        $data['role'] = "member";
        $data['user_type'] = "registered";
        $data["slug"] = $this->generate_uniqe_slug($this->input->post('email', true));
        $data['banned'] = 0;
