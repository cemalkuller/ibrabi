...
class Product_model extends Core_Model
{
    ...
    //get products
    public function get_user_products_all()
    {
        $this->db->where('products.is_deleted', 0);
    ...
    */
    ...
    public function insert_search_keyword($search)
    {
        $data['keyword'] = $search;
        $data['user_id'] = user()->id;
        $data['created_at'] = date('Y-m-d H:i:s');
        $this->db->insert('searchs', $data);
    }

    //latest searchs
    public function latest_searchs()
    {
        $this->db->where('user_id', user()->id);
        $query = $this->db->get('searchs');
        return $query->result();
    }
